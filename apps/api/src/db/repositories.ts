...
type TaskRow = {
  id: string; title: string; parent_id: string | null; due_date: string; recurrence_date: string | null; completed_at: string | null;
...
export function upsertTask(db: Db, task: Task): Task {
  ...
  db.prepare(`
    INSERT INTO tasks (id, title, parent_id, due_date, recurrence_date, completed_at, pinned, subtasks_collapsed, tag_id, notes, recurrence_json, sort_order, created_at, updated_at, deleted_at)
    VALUES (@id, @title, @parentId, @dueDate, @recurrenceDate, @completedAt, @pinned, @subtasksCollapsed, @tagId, @notes, @recurrence, @order, @createdAt, @updatedAt, @deletedAt)
    ON CONFLICT(id) DO UPDATE SET title=@title, parent_id=@parentId, due_date=@dueDate, recurrence_date=@recurrenceDate, completed_at=@completedAt,
  ...
  `).run({ ...task, recurrenceDate: task.recurrenceDate ?? null, tagId: tagIds[0] ?? null, pinned: task.pinned ? 1 : 0, subtasksCollapsed: task.subtasksCollapsed ? 1 : 0, recurrence: JSON.stringify(task.recurrence) });
